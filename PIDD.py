...
def negative_log_expert_probability(img_syn, label_syn, trained_model):
    probs = F.softmax(trained_model(img_syn), dim=1)
    label_probs = probs[torch.arange(len(img_syn)), label_syn]
...
def condense(args, logger, device='cuda'):
    ...
    it_test = np.arange(0, n_iter+1, args.record_iters).tolist()



    logger(f"\n PIDD: Start condensing for {n_iter} iteration")
    ...
    for it in range(n_iter):
        ...
        for c in range(nclass):
            ...
            img_real = img_aug[:n]
            with torch.no_grad():
                _, feat_tg = model_interval(img_aug[:n], return_features=True)
            _, feat = model_interval(img_aug[n:], return_features=True)
        ...
        for c in range(nclass):
            img_syn, label_syn = synset.sample(c, max_size=args.batch_syn_max)
            img_aug = aug(img_syn)
            sem_loss = args.lambdac * negative_log_expert_probability(img_aug, label_syn, model_final)
